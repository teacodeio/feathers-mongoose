...
class Service {
  ...
  update (id, data, params) {
    ...
    const discriminator = (params.query || {})[this.discriminatorKey] || this.discriminatorKey;
    const model = this.discriminators[discriminator] || this.Model;
    let findQuery = model.findOne({[this.id]: id});
    ...
    return findQuery
      ...
      .then((item) => {
        if(item) {
          ...
          // To allow $push for pushing items
          if(data['$push']) {
          ...
          if(this.overwrite && !hasDollarUpdate) {
        ...
        let findAndPupulateQuery = model.findOne({[this.id]: id});

        if (params && params.query && params.query.$populate) {
          findAndPupulateQuery = findAndPupulateQuery.populate(params.query.$populate);
        }

        return findAndPupulateQuery
          .lean(this.lean)
          .exec()
      })
      .then(select(params, this.id))
